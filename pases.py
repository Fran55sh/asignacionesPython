...
def cargar_archivos():
    archivos = filedialog.askopenfilenames(title="Selecciona archivos", filetypes=(("Archivos de texto", "*.txt"), ("Todos los archivos", "*.*")))
    
    # Copiar los archivos seleccionados a la carpeta del ejecutable
    carpeta_destino = os.path.dirname(os.path.abspath(__file__))
    for archivo in archivos:
        shutil.copy(archivo, carpeta_destino)
    
    # Ejecutar el código que utiliza los archivos
    # Aquí puedes incluir tu código que utiliza los archivos recién cargados

# Crear la ventana principal
ventana = tk.Tk()
ventana.title("Cargar Archivos")

# Botón para cargar archivos
boton_cargar = tk.Button(ventana, text="Cargar Archivos", command=cargar_archivos)
boton_cargar.pack()
...
# Recorrer la lista de líneas e imprimir cada línea
...
print(f"La columna '{nombre_de_columna}' tiene {cantidad_de_filas} filas con valores.")
...
# Convertir maecli_data en un DataFrame
...
def buscar_dni(row):
    ...
    filtro = df_intermedio['Nombres2'].str.replace(" ", "") == nombre2
...
# df_intermedio.to_excel('intermedio.xlsx', index=False)
print(f'DataFrame actualizado guardado en intermedio.xlsx')
...
dfFinal = dfFinal.iloc[:cantidad_de_filas] 


# Generar el nombre del archivo con el mes y el año actual

nombre_archivo = f"Asignacion {mes_actual} {ano_actual}.xlsx"

# Guardar el DataFrame en un archivo Excel
dfFinal.to_excel(nombre_archivo, index=False)
